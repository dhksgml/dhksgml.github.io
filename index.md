...
[카메라 관점]  
카메라는 하나를 사용하고, Unity의 포스트 프로세싱을 이용하여 카메라 효과를 꾸며 시각적인 재미를 주려고 합니다.
...
1. 시작화면, 게임연습 화면, 게임캐릭터 선택화면, 게임화면 총 4개의 화면이 있다.
2. 시작화면에는 게임시작과 게임방법 버튼 2개가 있다.
3. 게임시작 클릭 시, 게임캐릭터 선택화면으로 넘어간다.
4. 게임방법 클릭 시, 게임연습 화면으로 넘어간다.
5. 게임캐릭터 선택화면에서는 캐릭터 이미지가 담겨있는 4가지 버튼, 플레이어1이 캐릭터 선택완료 시 클릭할 버튼, 플레이어2가 캐릭터 선택완료 시 클릭할 버튼, 캐릭터가 선택되었을 때 캐릭터 모습을 보여줄 UI가 배치된다.
6. 플레이어가 4가지 캐릭터이미지 버튼 중에서 캐릭터를 선택하면, 선택한 캐릭터가 UI에 나타난다.
7. 캐릭터 선택완료 버튼을 누르면, 선택완료 버튼을 비활성화한다.
8. 두 플레이어 모두 캐릭터 선택을 마치면 게임화면으로 넘어간다.
9. 게임화면에는 플레이어1,2의 체력바, 일시정지 버튼, 콤보표시UI, 플레이어1,2의 캐릭터로 구성된다.
10. 두 플레이어가 선택한 캐릭터를 바탕으로, 게임화면에 캐릭터를 생성한다.
11. 같은 캐릭터를 선택한 경우, 2P 캐릭터의 색을 변경한다.
12. 플레이어1은 좌측에, 플레이어2는 우측에 배치된 캐릭터를 조작한다.
13. 플레이어가 이동키를 사용하면 캐릭터가 이동한다.
14. 1P와 2P가 서로 넘어가는 일이 일어나지 않도록 위치를 조정한다.
15. 플레이어가 주먹 공격 조작 시, 주먹 공격 애니메이션이 실행된다.
16. 플레이어가 발차기 공격 조작 시, 발차기 공격 애니메이션이 실행된다.
17. 플레이어가 방어 조작 시, 방어 동작 애니메이션이 실행된다.
18. 캐릭터 애니메이션에 따라 충돌처리할 히트박스를 활성화, 비활성화 한다.
19. 히트박스에 따라서 데미지 계산을 한다.
20. 플레이어 체력이 줄어들면, 체력UI의 게이지도 줄어든다.
21. 카메라는 플레이어1과 플레이어2를 비춘다.
22. 두 플레이어가 맵 밖으로 나가는 것을 방지한다.
23. 두 플레이어가 카메라 밖으로 나가는 것을 방지하기 위해, 두 플레이어 위치를 이용하여 카메라를 이동시킨다.
24. 충돌이 일어나면 camShakeDeg만큼 카메라를 흔드는 효과를 준다.
25. 두 플레이어 체력이 5보다 낮은 상황에서 두 캐릭터 거리가 1 이하인 경우, 서로 공격하고 있으면 슬로우 효과를 주고 줌인을 한다.
26. 라운드가 종료되면 승패를 판단하고 다음 라운드로 넘어간다.
27. 승리한 라운드를 표시하는 UI를 활성화한다.
28. 연속된 공격을 성공하면 콤보 UI를 활성화하고, 최종 공격으로 부터 일정 시간이 지나 콤보가 0이 되면 콤보 UI를 비활성화한다.
29. 게임 중 일시정지 버튼을 누를 시, 게임을 잠시 멈추고 버튼을 활성화 한다. 활성화 된 버튼을 누르면 일시정지가 해제된다.
30. 현재 라운드를 UI에 표시한다.
31. 2판을 이긴 플레이어가 있는 경우, 시작화면으로 넘어간다.
32. 충돌처리 발생 시, 충돌 위치에 파티클을 생성한다.
33. 충돌처리 발생 시, 효과음이 발생한다.
34. 포스트 프로세싱을 사용하여 슬로우 모션일 때, 색상을 변경한다.
35. 포스트 프로세싱을 사용하여 카메라가 흔들릴 때, 블러효과를 준다.
