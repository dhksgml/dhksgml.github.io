...
| 공격력 | p1AtkPower,p2AtkPower | 각 플레이어의 공격력.                           |
...
| (상단)왼쪽 발차기   | hk_left    | ↑+발차기 키를 1회 입력 시 공격.                                                                |
| (상단)오른쪽 발차기 | hk_right   | ↑+발차기 키를 연속 입력 시 공격.                                                               |
| (중단)왼쪽 발차기   | bk_left    | 발차기 키를 1회 입력 시 공격. 다른 방향키와 조합하지 않을 경우, 기본적으로 중단 발차기가 나감. |
| (중단)오른쪽 발차기 | bk_right   | 발차기 키를 연속 입력 시 공격.                                                                 |
| (하단)왼쪽 발차기   | lk_left    | ↓+발차기 키를 1회 입력 시 공격.                                                                |
| (하단)오른쪽 발차기 | lk_right   | ↓+발차기 키를 연속 입력 시 공격.                                                               |
| (상단)왼쪽 주먹     | hp_left    | 주먹 키를 1회 입력 시 공격. 다른 방향키와 조합하지 않을 경우, 기본적으로 상단 주먹이 나감.     |
| (상단)오른쪽 주먹   | hp_right   | 주먹 키를 연속 입력 시 공격.                                                                   |
| (중단)왼쪽 주먹     | bp_left    | ↓+주먹 키를 1회 입력 시 공격.                                                                  |
| (중단)오른쪽 주먹   | bp_right   | ↓+주먹 키를 연속 입력 시 공격.                                                                 |
...
- 플레이어1은 좌측에, 플레이어2는 우측에 배치된 캐릭터를 조작한다. -플레이어가 이동키를 사용하면 캐릭터가 이동한다.
