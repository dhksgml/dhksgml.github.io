...
17. 1P와 2P가 서로 넘어가는 일이 일어나지 않도록 위치를 조정한다.
18. 플레이어 주먹 공격 시 상태를 변경한다.
19. 플레이어 막기 시 상태를 변경한다.
20. 플레이어 발차기 시 상태를 변경한다.
...
25. 연속된 공격을 성공하면 콤보 UI를 활성화한다.
26. 콤보 횟수가 0이 되면 콤보 UI를 비활성화한다.
27. 일시정지 상태가 되면 버튼을 활성화 한다. 활성화 된 버튼을 누르면 일시정지가 해제된다.
28. 현재 라운드 수를 텍스트 UI에 표시한다.
...
8. 1P가 캐릭터를 선택하면 선택된 캐릭터 버튼 빨강색으로 변경하고 '1P'라는 텍스트를 띄운다.
9. 1P 선택된 캐릭터가 화면 왼쪽에 표시된다.
10. 1P가 캐릭터를 선택했으면 2P가 캐릭터를 선택한다.
11. 2P가 캐릭터를 선택하면 선택된 캐릭터 버튼을 초록색으로 변경하고 '2P'라는 텍스트를 띄운다.
12. 같은 캐릭터를 선택한 경우, 2P 캐릭터의 색을 변경한다.
13. 2P가 선택한 캐릭터를 화면 오른쪽에 표시한다.
14. 두 플레이어 모두 캐릭터 선택을 마치면 게임화면으로 넘어간다.
15. 두 플레이어가 선택한 캐릭터를 바탕으로, 게임화면에 캐릭터를 생성한다.
